...
function colorLine(line: string) {
  ...
  const parts = line.split(
    /(\b(?:import|package|public|private|protected|class|interface|record|return|new|if|else|for|while|try|catch|finally|throw|throws|static|final|void|int|long|boolean|String|var|true|false|null|synchronized|volatile)\b|'[^']*'|"[^"]*"|`[^`]*`)/g,
  );
  return parts.map((p, i) => {
    if (!p) return null;
    if (
      /^(import|package|public|private|protected|class|interface|record|return|new|if|else|for|while|try|catch|finally|throw|throws|static|final|void|int|long|boolean|String|var|true|false|null|synchronized|volatile)$/.test(
        p,
      )
    ) {
      return (

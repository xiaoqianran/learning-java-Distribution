...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      {
        title: "Java 分布式实战 · learning-java-Distribution",
      },
      {
        name: "description",
        content:
          "Java 分布式中文交互教程：并发·RPC·CAP·中间件·微服务·可靠性·测验与工坊。",
      },
    ],
    links: [
...
const CTP_BOOT =
  "(function(){try{var f=localStorage.getItem('java-dist-learn-ctp-flavor');var a=localStorage.getItem('java-dist-learn-ctp-accent');var okF=['mocha','macchiato','frappe','latte'];var okA=['green','mauve','blue','lavender','sapphire','teal','peach','pink'];if(okF.indexOf(f)<0)f='mocha';if(okA.indexOf(a)<0)a='peach';document.documentElement.setAttribute('data-ctp-flavor',f);document.documentElement.setAttribute('data-ctp-accent',a);}catch(e){document.documentElement.setAttribute('data-ctp-flavor','mocha');document.documentElement.setAttribute('data-ctp-accent','peach');}})();";

function RootDocument({ children }: { children: ReactNode }) {
  return (
    <html lang="zh-CN" data-ctp-flavor="mocha" data-ctp-accent="peach">
      <head>
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  const [expanded, setExpanded] = useState<Record<string, boolean>>(() => {
    ...
    for (const tr of orderedTracks()) init[tr] = tr === contTrack || tr === "并发基础";
    return init;
  });
